...
class ActDeact extends Base {
	...
	public static function upgrade_procedure() {
		...
		$version = \strval( \get_option( 'woocommerce-wishlist-version' ) );

		if ( !\version_compare( WW_VERSION, $version, '>' ) ) {
			return;
		}

		\update_option( 'woocommerce-wishlist-version', WW_VERSION );
		\delete_option( WW_TEXTDOMAIN . '_fake-meta' );
	}
